...
const ProductDetails = (props) => {
	...
	const handleDeleteProduct = () => {
		...
		fetch(`https://gifson-backend.onrender.com/deleteProducts/${id}`, {
			method: 'DELETE',
			headers: {
	...
	const onProductSubmit = (data, e) => {
		...
		fetch('https://gifson-backend.onrender.com/addProducts', {
			method: 'POST',
			headers: {
	...
	const onSubmit = (data) => {
		...
		fetch('https://gifson-backend.onrender.com/updateProduct', {
			method: 'POST',
			headers: {
				'Content-type': 'application/json'
			},
	...
	return (
		<div className="p-5 single-order">
			...
			<Modal toggle={() => setDeleteModal(false)} isOpen={deleteModal}>
				<h4 className="modal-title text-danger ml-4">Are you sure you want to delete this product?</h4>
