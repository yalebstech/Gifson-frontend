...
const Dashboard = props => {
  ...
  useEffect(() => {
    fetch(
      "https://gifson-backend.onrender.com/orders?email=" + auth.user.email
    )
      .then(res => res.json())
  ...
  return (
    <Container>
      <div className="d-flex justify-content-center mb-4">
        <h2 className="blue-color head-title mt-5">Your Order History</h2>
      </div>
      ...
      {order.map(item => (
        <div key={item._id}>
          {item.status === "Order Completed" && (
            <div className="jumbotron card card-image background-Order">
              <Row className="text-white px-4 d-flex align-items-center">
                ...
                <Col md={2}>
                  <Button
                    className="bg-color button-text ml-0 mt-1"
                    type="button"
                    onClick={() => openOrderModal(item._id)}
                    >
                    View Details
          ...
          >
            <div className="d-flex justify-content-center">
              <button
                className="btn mt-4 button-view"
                onClick={handlePrint}
               >
                Download Invoice
              </button>
